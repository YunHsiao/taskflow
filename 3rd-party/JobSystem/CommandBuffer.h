...
class alignas(64) CommandBuffer final
{
public:
    ...
    // 分配Command
    template <typename T>
    std::enable_if_t<std::is_base_of_v<Command, T>, T*>
    ...
    static void                             FreeChunksInFreeQueue(CommandBuffer* const mainCommandbuffer) noexcept;
...
private:
    ...
    public:
        ...
        void                                FreeByUser(CommandBuffer* const mainCommandbuffer) noexcept;
...
public:
    ...
    MemoryChunkSwitchCommand(CommandBuffer* const cb, uint8_t* const newChunk, uint8_t* const oldChunk) noexcept;
...
template <typename T>
std::enable_if_t<std::is_base_of_v<Command, T>, T*>
CommandBuffer::Allocate(uint32_t const count) noexcept
{
...
template <typename T>
T* CommandBuffer::AllocateAndCopy(uint32_t const count, void const* data) noexcept
{
...
template <typename T>
T* CommandBuffer::AllocateAndZero(uint32_t const count) noexcept
{
    T* const allocatedMemory = Allocate<T>(count);
